...
def classify_model_with_preprocessed_data_MLP(batch_size=64):
    """
    Evaluate a pre-trained MLP model using preprocessed features.
    ...
    model = MLP().to(device)
    model.load_state_dict(torch.load('MLP_ATT-esm+anova.pth', map_location=device))
    model.eval()
    ...
    result_df.to_csv('GUI/MLP-ATT-esm+anova_result.csv', index=False)
    ...
    metrics_df.to_csv('GUI/MLP-ATT-esm+anova_result_target.csv', index=False)
