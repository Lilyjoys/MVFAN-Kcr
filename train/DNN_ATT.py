...
class DNNAttention(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.attention = Attention(hidden_size // 4, hidden_size // 4)
...
for train_index, test_index in kf.split(X):
    ...
    print(f"Fold {fold} - Train Pos: {y_train.sum()}, Neg: {(y_train == 0).sum()}")
    print(f"Fold {fold} - Test Pos: {y_test.sum()}, Neg: {(y_test == 0).sum()}")

    input_size = X_train.shape[1]
    hidden_size = 128
    output_size = 1
    model = DNNAttention(input_size, hidden_size, output_size)
...
results_df.to_csv('dnn_attention_fold.csv', index=False)
